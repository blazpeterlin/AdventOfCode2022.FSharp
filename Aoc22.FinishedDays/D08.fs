...
let countNumTrees (mp: Map<(int*int),int>) (x0,y0) (ix,iy) (edgeX,edgeY) =
    ...
    let forLine = 
        coords
        |> List.fold (fun (isBlocked,lst) (x,y) -> 
            let nextElt = mp[x,y]
            let nextLst = if (not isBlocked) (*&& nextElt <= blockedValue*) then (x,y)::lst else lst
            let nextIsBlocked = isBlocked || nextElt >= blockedValue
            nextIsBlocked,nextLst
        ) (false,[])
    let numTrees = forLine |> snd |> List.length
    numTrees

let scenicScore (mp: Map<(int*int),int>) (x0,y0) =
    let maxY = mp |> Map.keys |> List.ofSeq |> List.map snd |>List.max
    let maxX = mp |> Map.keys |> List.ofSeq |> List.map fst |>List.max
